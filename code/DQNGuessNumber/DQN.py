...
import torch
from code.QlearningTest import Net
...
class DQN(object):
    def __init__(self, n_states, n_actions, n_hidden, batch_size, lr, epsilon, gamma, target_replace_iter,
        ...
        self.memory = np.zeros((memory_capacity,
                                n_states * 2 + 2))  # 每個 memory 中的 experience 大小為 (state + next state + reward + action)
    ...
    def choose_action(self, state):
        x = torch.unsqueeze(torch.FloatTensor(state).cuda(), 0)
        ...
        if np.random.uniform() < self.epsilon:  # 隨機
            ...
            action = torch.max(actions_value, 1)[1].data.cpu().numpy()[0]  # 挑選最高分的 action
    ...
    def learn(self):
        ...
        b_state = torch.FloatTensor(b_memory[:, :self.n_states]).cuda()
        b_action = torch.LongTensor(b_memory[:, self.n_states:self.n_states + 1].astype(int)).cuda()
        b_reward = torch.FloatTensor(b_memory[:, self.n_states + 1:self.n_states + 2]).cuda()
        b_next_state = torch.FloatTensor(b_memory[:, -self.n_states:]).cuda()
        ...
        q_target = b_reward + self.gamma * q_next.max(1)[0].view(self.batch_size,
                                                                 1)  # 計算這些 experience 當下 target net 所得出的 Q value
